...
@pytest.mark.parametrize("provider", ["ollama", "vllm", "llamacpp", "local"])
def test_run_doctor_accepts_alias_to_noncataloged_sentinel_provider(monkeypatch, tmp_path, provider):
    """Regression: providers that normalize to a non-cataloged sentinel

    ("ollama" -> "custom", "vllm"/"llamacpp"/literal "local" -> "local") must
    not be flagged as unrecognised. resolve_provider_full() legitimately
    returns None for these — "custom" and "local" are deliberately absent
    from models.dev/overlays — which used to be misread as "unknown provider".
    """
    home = tmp_path / ".hermes"
    home.mkdir(parents=True, exist_ok=True)
    (home / "config.yaml").write_text(
